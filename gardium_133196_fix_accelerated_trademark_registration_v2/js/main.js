...
var ClientSize = {
	...
	initialize: function () {
		var self = this;
		//this.clientReview = this.$el.find('.js-client__review-wrap');
	...
	renderMode: function () {
		var self = this;
		var lgHeight = 190;
		var widthClient = this.$el.outerWidth();
		if($(window).outerWidth() < 768) {
			this.$el.outerHeight(widthClient);

		} else {
			this.$el.outerHeight(lgHeight);
		}
	}
};

App.Control.install(ClientSize);
	...
	initialize: function () {
		...
		this.slider = this.$el.bxSlider({
			...
			onSlideBefore: function (slideElement, oldIndex, newIndex) {
				// проверяем является ли слайдер последним
				...
				/*if (slideElement.hasClass('js-process-slider__slide--no-text')) {
					$('.js-process-slider__btn-holder').addClass('process-slider__btn-holder--no-text-intend');
				} else {
					$('.js-process-slider__btn-holder').removeClass('process-slider__btn-holder--no-text-intend');
				}*/
